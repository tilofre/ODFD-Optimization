...
def handle_hindsight_analysis(timestart, order_queue, new_orders_this_step, couriers, constants, hindsight_cache):
    ...
    #print(f"Hindsight-Analyse: {len(potential_batches)} profitable Bündel gefunden. Prüfe alle Optionen...")
    ...
    for batch_info in potential_batches:
        ...
        #print(f"-> Prüfe Bündel mit Orders {hindsight_batch_info['orders']}. Potenzial: {batch_info['saving_minutes']:.2f} min.")

        # 'new_orders_this_step' wird hier Teil der relevanten Aufträge
        ...
        if bringer_result and bringer_result.get('decision') == "Ja":
            print(f"--> Entscheidung: Bringer-Kurier für dieses Bündel gestartet! Spart {bringer_result['saving_minutes']:.2f} min.")
            ...
            if bringer_courier and target_courier:
                ...
                # WICHTIG: Auftrag aus BEIDEN Listen entfernen (falls er dort war)
    ...
    # WICHTIG: 'new_orders_this_step' (die modifizierte Liste) zurückgeben
    return couriers, order_queue, new_orders_this_step, hindsight_cache, used_courier_ids_this_step, processed_order_ids_this_step
...
def evaluate_bringer_courier_option(hindsight_batch_info, relevant_orders, couriers, timestart, constants, used_courier_ids=None):
    ...
    Returns:
        dict: A decision dictionary ({"decision": "Ja" or "Nein", ...}).
    """
    ...
    if unassigned_order is None: 
        return {"decision": "Nein", "reason": "No unassigned order found in batch."}
    ...
    if assigned_order_id is None: 
        return {"decision": "Nein", "reason": "No assigned order ID found in batch."}
    ...
    if target_courier is None: 
        return {"decision": "Nein", "reason": "Target courier is no longer busy or not found."}

    # Check capacity of the target courier
    MAX_DELIVERIES = 3
    if target_courier.active_deliveries >= MAX_DELIVERIES: 
        return {"decision": "Nein", "reason": f"Target courier at max capacity ({target_courier.active_deliveries}/{MAX_DELIVERIES})."}
    ...
    if not idle_couriers: 
        return {"decision": "Nein", "reason": "No idle couriers available for bringer role."}
    ...
    if max_eta_shift <= ETA_SHIFT_THRESHOLD_SECONDS and not is_any_order_late_in_self_pickup:
        ...
        return {"decision": "Nein", "reason": f"Self-pickup is efficient enough (ETA-Shift: {max_eta_shift/60:.1f}min)."}
    ...
    if intercept_point is None: 
        return {"decision": "Nein", "reason": "No intercept point found for bringer action."}
    ...
    if is_time_saving_positive and not is_any_order_late:
        ...
        return {"decision": "Nein", "reason": reason}
